...
module.exports = class WallboxAPI {
  ...
  */
  ...
  /**
   * Sets charger energy cost
   *
   * @param {string} chargerId - Identifier of the target charger
   * @param {JSON} data - The charger property data that needs to be updated
   * @returns {Promise} Promise object represents the result of setting charger properties
   */
  async _setChargerEnergyCost(chargerId, data) {
  ...
  this.homey.log('Succesfully set energy cost: ', data);
 ...
 /**
   ...
   *
   * @param {string} chargerId - Identifier of the target charger
   * @param {int} energyCost - Energy cost
   * @returns {Promise} Promise object represents the result of setting energy cost
   */
  async setEnergyCost(chargerId, energyCost) {
    await this._setChargerEnergyCost(chargerId, { energyCost: energyCost });
  }

  async getSessionsList() {
    const url = `${this._baseUrl}v4/sessions/stats`;
    const res = await fetch(url, { headers: this._headers });

    if (!res.ok)
      throw new Error(res.status);

    this.homey.log('Succesfully got sessions list');
    return await res.json();
  }
}
